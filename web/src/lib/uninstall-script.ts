// The manual cleanup script the force-disconnect dialog hands out (SIGMA-205).
//
// This is a VERBATIM copy of agent/packaging/uninstall.sh — the same file that
// ships as a release asset — and decommission.test.ts asserts the two are byte
// identical. That test is the point of this module existing at all: a cleanup
// script that drifts from the one the product actually ships is worse than no
// script, because it is run by someone who has already been told the graceful
// path failed, on a host they are trying to get back.
//
// It lives here as a literal rather than being read from disk because the
// dialog is a client component: there is no filesystem at the point of render.
// The backslashes in the body are TypeScript escapes protecting the script's
// shell variable expansions from template-literal interpolation; they are not
// part of the script, which is why the drift test compares VALUES and not
// source text.
export const MANUAL_UNINSTALL_SCRIPT = `#!/usr/bin/env bash
