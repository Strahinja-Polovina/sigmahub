...
import { chargeOrgMail, spendOrgMail } from "../mail-budget";
import {
  INVITE_TTL_MS,
  appBaseUrl,
  hashInviteToken,
...
export async function inviteMember(input: {
...
}): Promise<ActionResult<{ inviteUrl: string; delivered: boolean; throttled: boolean }>> {
  return attempt(async () => {
    ...
    if (alreadyMember) throw new Refusal("That person is already a member.");
    ...
    // The org's outbound-mail budget (SIGMA-365), scoped per org so one abuser
    // cannot spend another tenant's allowance.
    //
    // It refuses the MAIL, not the invite. Refusing the invite here was the first
    // version of this limit, and it produced the exact dead end this review
    // series is about: the message told the admin to copy the invite link, and
    // because the refusal came BEFORE the insert there was no invitation and no
    // link to copy. So the invitation is always created; when the budget is spent
    // the send is skipped and the link comes back for the admin to relay — the
    // same degraded path a deployment with no transport already takes, bounded
    // for the same reason, because nothing was sent.
    const throttledFor = await spendOrgMail(input.orgId);
    ...
    const { delivered } =
      throttledFor === null
        ? await sendInviteEmail({ to: email, orgName: await orgName(input.orgId), role, url })
        : { delivered: false };
    await writeAudit({
      orgId: input.orgId,
      actor: actor.name,
    ...
    // `throttled` separates "this deployment sends no mail" from "you have hit
    // the hourly limit" at the dialog, which would otherwise tell a perfectly
    // well-configured operator that their SMTP is not set up.
    return { inviteUrl: url, delivered, throttled: throttledFor !== null };
  });
}
...
export async function resendInvite(input: {
  orgId: string;
  invitationId: string;
}): Promise<ActionResult<{ inviteUrl: string; delivered: boolean }>> {
  return attempt(async () => {
    const actor = await requireOrgAdmin(input.orgId);
    const [inv] = await db
      .select({
    ...
    if (!inv || inv.status !== "pending") throw new Refusal("No pending invite to resend.");
...
export async function revokeInvite(input: { orgId: string; invitationId: string }): Promise<ActionResult<object>> {
  return attempt(async () => {
    const actor = await requireOrgAdmin(input.orgId);
    const [inv] = await db
      ...
      .from(s.invitations)
      .where(and(eq(s.invitations.id, input.invitationId), eq(s.invitations.orgId, input.orgId)));
    if (!inv || inv.status !== "pending") throw new Refusal("No pending invite to revoke.");
...
export async function acceptInvite(input: { token: string }): Promise<ActionResult<{ orgId: string }>> {
  return attempt(async () => {
    ...
    const orgId = await db.transaction(async (tx) => {
      ...
      if (rejection) throw new Refusal(inviteRejectionMessage(rejection));
    ...
    const target = await db.transaction(async (tx) => {
      if (role !== "Org Admin") {
        ...
        if (admins.length <= 1 && admins.some((a) => a.userId === input.userId)) {
          throw new Refusal("An organization needs at least one admin.");
        }
      }
      const [t] = await tx
...
export async function removeMember(input: { orgId: string; userId: string }): Promise<ActionResult<object>> {
  return attempt(async () => {
    const actor = await requireOrgAdmin(input.orgId);
    if (input.userId === actor.id) {
      throw new Refusal("You can’t remove yourself from an organization.");
    }
