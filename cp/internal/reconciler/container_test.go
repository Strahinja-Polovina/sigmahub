...
func TestRenderAppResourceFansIntoContainerOps(t *testing.T) {
	...
	ops, hash := renderOps(specs, nil, nil)
	if hash == "" {
...
func TestRenderDedupsProjectNetwork(t *testing.T) {
	...
	ops, _ := renderOps([]store.ResourceSpec{mk("res_a"), mk("res_b")}, nil, nil)
	netCount := 0
	for _, op := range ops {
...
func TestRenderAppendsDestructiveOps(t *testing.T) {
	...
	ops, _ := renderOps(nil, pending, nil)
