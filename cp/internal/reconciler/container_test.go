...
func TestRenderAppResourceFansIntoContainerOps(t *testing.T) {
	...
	ops, hash := renderOps("srv_t", specs, nil, nil, store.HostHardening{}, nil, ACMEConfig{})
	if hash == "" {
...
func TestRenderDedupsProjectNetwork(t *testing.T) {
	...
	ops, _ := renderOps("srv_t", []store.ResourceSpec{mk("res_a"), mk("res_b")}, nil, nil, store.HostHardening{}, nil, ACMEConfig{})
	netCount := 0
	for _, op := range ops {
...
func TestRenderStubFallback(t *testing.T) {
	...
	ops, _ := renderOps("srv_t", specs, nil, nil, store.HostHardening{}, nil, ACMEConfig{})
...
func TestRenderAppendsDestructiveOps(t *testing.T) {
	...
	ops, _ := renderOps("srv_t", nil, pending, nil, store.HostHardening{}, nil, ACMEConfig{})
