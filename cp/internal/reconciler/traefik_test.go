...
func TestRenderTraefikOpOnlyForProxyRole(t *testing.T) {
	...
	ops, _ := renderOps("srv_np", specs, nil, nil, store.HostHardening{ProxyRole: false}, nil, nil, nil, acme)
	...
	ops, _ = renderOps("srv_p", specs, nil, nil, store.HostHardening{ProxyRole: true}, nil, nil, nil, acme)
...
func TestRenderAppLabelsWhenDomainAttached(t *testing.T) {
	...
	ops, _ := renderOps("srv_p", specs, nil, nil, store.HostHardening{ProxyRole: true}, domains, nil, nil, ACMEConfig{})
...
func TestTraefikChallengeTypeFromDomains(t *testing.T) {
	...
	ops, _ := renderOps("srv_p", specs, nil, nil, store.HostHardening{ProxyRole: true}, allAlpn, nil, nil, ACMEConfig{})
	...
	ops, _ = renderOps("srv_p", specs, nil, nil, store.HostHardening{ProxyRole: true}, mixed, nil, nil, ACMEConfig{})
	op, _ = opByID(ops, "proxy:traefik:srv_p")
	_ = json.Unmarshal(op.Spec, &ts)
...
func TestRenderNoLabelsWithoutDomain(t *testing.T) {
	spec, _ := json.Marshal(map[string]any{"image": "nginx", "ports": []map[string]any{{"container": 8080}}})
	specs := []store.ResourceSpec{{ResourceID: "res_a", ProjectID: "proj_x", Kind: "app", Spec: spec}}
	ops, _ := renderOps("srv_p", specs, nil, nil, store.HostHardening{ProxyRole: true}, nil, nil, nil, ACMEConfig{})
	ctr, _ := opByID(ops, "res:res_a")
	var cs containerOpSpec
	_ = json.Unmarshal(ctr.Spec, &cs)
	if cs.Labels != nil {
...
func TestTraefikDeterministicHash(t *testing.T) {
	...
	_, h1 := renderOps("srv_p", specs, nil, nil, store.HostHardening{ProxyRole: true}, domains, nil, nil, ACMEConfig{Email: "x@y.z"})
	_, h2 := renderOps("srv_p", specs, nil, nil, store.HostHardening{ProxyRole: true}, domains, nil, nil, ACMEConfig{Email: "x@y.z"})
	if h1 != h2 {
