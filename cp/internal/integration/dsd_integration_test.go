...
func testStore(t *testing.T) (*store.Store, ed25519.PrivateKey) {
	...
	for _, tbl := range []string{"server_dsd", "deploy_logs", "deployments", "builds", "deploy_requests", "git_branch_map", "webhook_deliveries", "git_connections", "domains", "dns_provider_credentials", "preview_environments", "backup_runs", "backup_policies", "backup_targets", "db_credentials", "secrets", "org_deks", "env_servers", "resources", "environments", "projects", "agent_tokens", "bootstrap_tokens", "service_tokens", "server_hardening", "servers", "cp_audit_log", "cp_secrets"} {
