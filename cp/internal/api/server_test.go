...
func (f *fakeDomain) PurgeOrg(_ context.Context, orgID string) (store.PurgeResult, error) {
	f.purged = append(f.purged, orgID)
	return store.PurgeResult{Deleted: map[string]int64{"servers": 1}, Tenant: 7}, nil
}
...
func (f *fakeDomain) ListArchivedRepoKeys(context.Context, string) ([]store.ArchivedRepoKey, error) {
	return nil, nil
}
func (f *fakeDomain) ExportRepoKey(_ context.Context, _, _, _ string) (string, error) {
	return "", store.ErrNotFound
}
