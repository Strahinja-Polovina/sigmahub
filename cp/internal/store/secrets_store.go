...
// UpdateSecretValue re-seals an existing secret's value under the org's ACTIVE
// DEK, keeping the row — its id, name, scope and env/file mode — intact.
//
// This is the rotation path (SIGMA-264). Without it the only way to change a
// value was delete-then-create, which mints two rounds of config deployments:
// the first re-rolls every dependent app WITHOUT the variable, so a live
// service restarts missing its credential before the replacement lands. Keeping
// the id also keeps every ref that names this secret pointing at the same row.
//
// The AAD is unchanged (it binds org|project|secret id, none of which move), and
// a fresh nonce is generated by gcmSeal for the new ciphertext — reusing a nonce
// under the same DEK would be a GCM key-recovery break. Audited like every other
// secret mutation.
func (s *Store) UpdateSecretValue(ctx context.Context, orgID, secretID, value, actor string) (Secret, error) {
	tx, err := s.Pool.Begin(ctx)
	if err != nil {
		return Secret{}, err
	}
	...
	var sec Secret
	err = tx.QueryRow(ctx, `
		SELECT id, project_id, environment_id, name, env_var, created_by
		  FROM secrets WHERE org_id = $1 AND id = $2 FOR UPDATE`, orgID, secretID).
		Scan(&sec.ID, &sec.ProjectID, &sec.EnvironmentID, &sec.Name, &sec.EnvVar, &sec.CreatedBy)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	nonce, ct, err := gcmSeal(dek, secretAAD(orgID, sec.ProjectID, sec.ID), []byte(value))
	if err != nil {
		return Secret{}, err
	}
	if _, err := tx.Exec(ctx, `
		UPDATE secrets SET ciphertext = $3, nonce = $4, dek_id = $5
		 WHERE org_id = $1 AND id = $2`, orgID, secretID, ct, nonce, dekID); err != nil {
		return Secret{}, fmt.Errorf("update secret: %w", err)
	}
	if err := auditTx(ctx, tx, orgID, actor, "Secret value updated", sec.Name); err != nil {
		return Secret{}, err
	}
	if err := tx.Commit(ctx); err != nil {
		return Secret{}, err
